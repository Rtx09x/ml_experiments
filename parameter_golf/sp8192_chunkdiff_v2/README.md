# SP8192 Causal ChunkDiff V2

## What this is

V2 keeps the top SP8192 causal leaderboard stack for legal BPB scoring, and makes the diffusion part more explicit than V1.

Evaluation still uses the causal `forward_logits()` path only. Diffusion is training-time only.

## Diffusion-specific pieces

V2 adds these over V1:
...
Default V2 is based on the 65M top-stack lane:
...
git clone -b boobie/sp8192-chunkdiff-v2 https://github.com/Rtx09x/ml_experiments.git
chmod +x ml_experiments/parameter_golf/sp8192_chunkdiff_v2/runpod_8xh100.sh
...
VAL_LOSS_EVERY=1000 \
TRAIN_LOG_EVERY=100 \
./ml_experiments/parameter_golf/sp8192_chunkdiff_v2/runpod_8xh100.sh
```
...
```bash
CHUNKDIFF_WEIGHT=0.03 CHUNKDIFF_EVERY=2
```
...
```text
model_params:...
chunkdiff:enabled:1 weight:0.05 chunk_size:64 mask:[0.2,0.9] every:1 min_step:0 seq_len:256 steps:8 time_cond:1 denoise_adapter:1
```
