...
class CausalSelfAttention(nn.Module):
	...
	def forward(self,x,attn_mask=None):
		...
		if attn_mask is None:y=flash_attn_3_func(q,k,v,causal=True)
		else:
			kk=k;vv=v
			if self.num_heads!=self.num_kv_heads:
				group=self.num_heads//self.num_kv_heads;kk=kk.repeat_interleave(group,dim=2);vv=vv.repeat_interleave(group,dim=2)
			y=F.scaled_dot_product_attention(q.transpose(1,2),kk.transpose(1,2),vv.transpose(1,2),attn_mask=attn_mask,dropout_p=0.,is_causal=False).transpose(1,2)
...
class Block(nn.Module):
	...
	def forward(self,x,x0,attn_mask=None):
		mix=self.resid_mix.to(dtype=x.dtype);x_in=mix[0][None,None,:]*x+mix[1][None,None,:]*x0;attn_out=self.attn(self.attn_norm(x_in)*self.ln_scale_factor,attn_mask=attn_mask)
...
class GPT(nn.Module):
	def __init__(self,h):
		...
		self.tie_embeddings=h.tie_embeddings;self.tied_embed_init_std=h.tied_embed_init_std;self.logit_softcap=h.logit_softcap;self.chunkdiff_steps=max(1,h.chunkdiff_steps);self.chunkdiff_time_cond=h.chunkdiff_time_cond;self.chunkdiff_denoise_adapter_enabled=h.chunkdiff_denoise_adapter;self.tok_emb=nn.Embedding(h.vocab_size,h.embedding_dim);self.blockdiff_mask_embed=nn.Parameter(torch.zeros(h.embedding_dim,dtype=torch.float32));self.chunkdiff_time_embed=nn.Embedding(self.chunkdiff_steps,h.embedding_dim);self.chunkdiff_denoise_adapter=CastedLinear(h.embedding_dim,h.embedding_dim,bias=False);self.chunkdiff_denoise_adapter._zero_init=True
	...
	def forward_logits(self,input_ids):
		...
		return self.logit_softcap*torch.tanh(logits_proj/self.logit_softcap)+(self.blockdiff_mask_embed.sum()+self.chunkdiff_time_embed.weight.sum()+self.chunkdiff_denoise_adapter.weight.sum()).to(dtype=logits_proj.dtype)*0.
	def chunkdiff_loss(self,input_ids,chunk_size,mask_min,mask_max):
		...
		num_chunks=seqlen//chunk_size;chunk_idx=torch.randint(1,num_chunks,(bsz,1),device=dev);chunk_start=chunk_idx*chunk_size;pos=torch.arange(seqlen,device=dev)[None,:];in_chunk=(pos>=chunk_start)&(pos<chunk_start+chunk_size)
		t=torch.randint(0,self.chunkdiff_steps,(bsz,1),device=dev);den=max(self.chunkdiff_steps-1,1);noise_frac=t.float()/den
		mask_min=max(0.,min(float(mask_min),1.));mask_max=max(mask_min,min(float(mask_max),1.));ratios=mask_min+(mask_max-mask_min)*noise_frac;mask_positions=in_chunk&(torch.rand((bsz,seqlen),device=dev)<ratios)
		if not bool(mask_positions.any()):mask_positions=in_chunk&(pos==chunk_start)
		qpos=torch.arange(seqlen,device=dev)[None,:,None];kpos=torch.arange(seqlen,device=dev)[None,None,:];cs=chunk_start[:,:,None];ce=cs+chunk_size;q_in=(qpos>=cs)&(qpos<ce);k_in=(kpos>=cs)&(kpos<ce);attn_mask=((kpos<=qpos)|(q_in&k_in))[:,None,:,:]
		x=self.tok_emb(input_ids);time_x=self.chunkdiff_time_embed(t.squeeze(1)).to(dtype=x.dtype)[:,None,:] if self.chunkdiff_time_cond else torch.zeros((bsz,1,x.size(-1)),device=dev,dtype=x.dtype);x=x+time_x;x=torch.where(mask_positions[...,None],self.blockdiff_mask_embed.to(dtype=x.dtype,device=dev)[None,None,:]+time_x,x);x=F.rms_norm(x,(x.size(-1),))
		if self.embed_proj is not None:x=self.embed_proj(x)
		x0=x;skips=[];enc_iter=self.encoder_indices if self.looping_active else range(self.num_encoder_layers);dec_iter=self.decoder_indices if self.looping_active else range(self.num_encoder_layers,self.num_encoder_layers+self.num_decoder_layers)
		for i in enc_iter:x=self.blocks[i](x,x0,attn_mask=attn_mask);skips.append(x)
		for(skip_idx,i)in enumerate(dec_iter):
			if skip_idx<self.num_skip_weights and skips:
				scaled_skip=self.skip_weights[skip_idx].to(dtype=x.dtype)[None,None,:]*skips.pop()
				if self.skip_gates is not None:g=torch.sigmoid(self.skip_gates[skip_idx].to(dtype=x.dtype))[None,None,:];x=torch.lerp(scaled_skip,x,g)
		...
		x=self.final_norm(x)
		if self.head_proj is not None:x=self.head_proj(x)
		if self.chunkdiff_denoise_adapter_enabled:x=x+self.chunkdiff_denoise_adapter(F.silu(x))
		if self.tie_embeddings:logits_proj=F.linear(x,self.tok_emb.weight)
		else:logits_proj=self.lm_head(x)
		logits=self.logit_softcap*torch.tanh(logits_proj/self.logit_softcap)
		return F.cross_entropy(logits[mask_positions].float(),input_ids[mask_positions],reduction='mean')
...
def _sparse_target_for_step(h,step):
	...
	if step<h.sparse_start_step:return 0.
	if h.sparse_end_step<=h.sparse_start_step:return h.sparse_target
	frac=min(max((step-h.sparse_start_step)/(h.sparse_end_step-h.sparse_start_step),0.),1.)
	return h.sparse_target*frac
@torch.no_grad()
def apply_magnitude_sparsity_(model,h,target):
	include={x.strip()for x in h.sparse_include.split(',')if x.strip()}
	params=[]
	for name,p in model.named_parameters():
		if p.ndim<2 or classify_param(name)not in include:continue
		params.append(p)
...
def pack_int4_signed(q):
	q=(q.to(torch.int16)+8).clamp_(0,15).to(torch.uint8).flatten()
	if q.numel()%2:q=torch.cat([q,torch.zeros(1,dtype=torch.uint8)])
	return(q[0::2]|(q[1::2]<<4)).contiguous()
def unpack_int4_signed(packed,shape):
	u=packed.to(torch.uint8).flatten();n=int(torch.tensor(shape).prod().item());lo=u&15;hi=(u>>4)&15;out=torch.empty(u.numel()*2,dtype=torch.int8);out[0::2]=lo.to(torch.int8)-8;out[1::2]=hi.to(torch.int8)-8;return out[:n].reshape(tuple(int(x)for x in shape))
def gptq_mixed_quantize(state_dict,hessians,h):
	...
	for(name,tensor)in state_dict.items():
		...
		cs=h.embed_clip_sigmas if'tok_emb'in name else h.matrix_clip_sigmas;bits=h.embed_bits if'tok_emb'in name else h.matrix_bits;q,s=gptq_quantize_weight(t,hessians[name],clip_sigmas=cs,clip_range=2**(bits-1)-1)
		if bits==4:
			result[name+'.q4']=pack_int4_signed(q);result[name+'.shape']=torch.tensor(q.shape,dtype=torch.int32);result[name+'.scale']=s;meta[name]='gptq (packed int4)'
		else:
...
def dequantize_mixed(result,meta,template_sd):
	...
	for(name,orig)in template_sd.items():
		...
		if'passthrough'in info:
			...
			out[name]=t;continue
		if'packed int4'in info:q=unpack_int4_signed(result[name+'.q4'],result[name+'.shape'])
		else:q=result[name+'.q']
		s=result[name+'.scale']
...
def serialize(h,base_model,code):
	...
	if h.is_main_process:
		torch.save(base_model.state_dict(),h.model_path);torch.save(base_model.state_dict(),'final_model_pre_quant.pt');model_bytes=os.path.getsize(h.model_path);log(f"Serialized model: {model_bytes} bytes");log("Saved pre-quant model checkpoint: final_model_pre_quant.pt");log(f"Code size: {code_bytes} bytes")
...
def train_model(h,device,val_data):
	...
	log(f"model_params:{sum(p.numel()for p in base_model.parameters())}");log(f"chunkdiff:enabled:{int(h.chunkdiff_enabled)} weight:{h.chunkdiff_weight} chunk_size:{h.chunkdiff_chunk_size} mask:[{h.chunkdiff_mask_min},{h.chunkdiff_mask_max}] every:{h.chunkdiff_every} min_step:{h.chunkdiff_min_step} seq_len:{h.chunkdiff_seq_len} steps:{h.chunkdiff_steps} time_cond:{int(h.chunkdiff_time_cond)} denoise_adapter:{int(h.chunkdiff_denoise_adapter)}");optimizers=Optimizers(h,base_model);train_loader=ShuffledSequenceLoader(h,device);max_wallclock_ms=1e3*h.max_wallclock_seconds if h.max_wallclock_seconds>0 else None
	...
	def step_fn(step,lr_scale):
		optimizers.zero_grad_all();train_loss=torch.zeros((),device=device);train_chunkdiff_loss=torch.zeros((),device=device)
		for micro_step in range(h.grad_accum_steps):
			if h.distributed:model.require_backward_grad_sync=micro_step==h.grad_accum_steps-1
			x,y=train_loader.next_batch(h.train_batch_tokens,h.grad_accum_steps)
			with torch.autocast(device_type='cuda',dtype=torch.bfloat16,enabled=True):
				ar_loss=model(x,y);use_chunkdiff=h.chunkdiff_enabled and h.chunkdiff_weight>0 and step>=h.chunkdiff_min_step and h.chunkdiff_every>0 and step%h.chunkdiff_every==0
				chunkdiff_x=x[:, :min(x.size(1), h.chunkdiff_seq_len)].contiguous() if use_chunkdiff else x
				chunkdiff_loss=base_model.chunkdiff_loss(chunkdiff_x,h.chunkdiff_chunk_size,h.chunkdiff_mask_min,h.chunkdiff_mask_max) if use_chunkdiff else ar_loss.new_zeros(())
				loss=ar_loss+h.chunkdiff_weight*chunkdiff_loss
			train_loss+=loss.detach();train_chunkdiff_loss+=chunkdiff_loss.detach();(loss/h.grad_accum_steps).backward()
		train_loss/=h.grad_accum_steps;train_chunkdiff_loss/=h.grad_accum_steps;frac=min(step/h.muon_momentum_warmup_steps,1.)if h.muon_momentum_warmup_steps>0 else 1.;muon_momentum=(1-frac)*h.muon_momentum_warmup_start+frac*h.muon_momentum
		...
		optimizers.step();return train_loss,train_chunkdiff_loss
	if h.warmup_steps>0:
	...
	while True:
		...
		train_loss,chunkdiff_loss=step_fn(step,scale)
		if h.sparse_enabled and h.sparse_every>0 and step%h.sparse_every==0:
			sparse_target=_sparse_target_for_step(h,step)
			if sparse_target>0:
				zeros,total=apply_magnitude_sparsity_(base_model,h,sparse_target);actual=zeros/max(total,1)
				log(f"sparsity:step:{step} target:{sparse_target:.3f} actual:{actual:.3f} include:{h.sparse_include}")
		with torch.no_grad():
		...
		if should_log_train:tok_per_sec=step*h.train_batch_tokens/(approx_training_time_ms/1e3);log(f"{step}/{h.iterations} train_loss: {train_loss.item():.4f} chunkdiff_loss: {chunkdiff_loss.item():.4f} train_time: {approx_training_time_ms/60000:.1f}m tok/s: {tok_per_sec:.0f}")
