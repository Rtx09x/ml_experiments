...
EXPERIMENT_DIR="${EXPERIMENT_REPO_DIR}/parameter_golf/sp8192_chunkdiff_v3"
RUN_ID="${RUN_ID:-sp8192_chunkdiff_v3_8xh100}"
RUN_ROOT="${RUN_ROOT:-${WORKSPACE_ROOT}/sp8192_chunkdiff_v3_runs}"
...
mkdir -p "${PARAMETER_GOLF_DIR}/working/sp8192_chunkdiff_v3"
cp "${EXPERIMENT_DIR}/train_gpt.py" "${PARAMETER_GOLF_DIR}/working/sp8192_chunkdiff_v3/train_gpt.py"
cp "${EXPERIMENT_DIR}/README.md" "${PARAMETER_GOLF_DIR}/working/sp8192_chunkdiff_v3/README.md"
cp "${EXPERIMENT_DIR}/runpod_8xh100.sh" "${PARAMETER_GOLF_DIR}/working/sp8192_chunkdiff_v3/runpod_8xh100.sh"
...
rm -f final_model.pt final_model_pre_quant.pt final_model.int4.ptz final_model.int6.ptz
mkdir -p logs

export RUN_ID
export DATA_DIR="${DATA_DIR:-./data/}"
export VOCAB_SIZE="${VOCAB_SIZE:-8192}"
export MODEL_DIM="${MODEL_DIM:-896}"
...
export VAL_BATCH_TOKENS="${VAL_BATCH_TOKENS:-262144}"
...
export CHUNKDIFF_WEIGHT="${CHUNKDIFF_WEIGHT:-0.03}"
export CHUNKDIFF_CHUNK_SIZE="${CHUNKDIFF_CHUNK_SIZE:-64}"
export CHUNKDIFF_MASK_MIN="${CHUNKDIFF_MASK_MIN:-0.20}"
export CHUNKDIFF_MASK_MAX="${CHUNKDIFF_MASK_MAX:-0.90}"
export CHUNKDIFF_EVERY="${CHUNKDIFF_EVERY:-1}"
export CHUNKDIFF_MIN_STEP="${CHUNKDIFF_MIN_STEP:-0}"
export CHUNKDIFF_SEQ_LEN="${CHUNKDIFF_SEQ_LEN:-128}"
export CHUNKDIFF_STEPS="${CHUNKDIFF_STEPS:-8}"
export CHUNKDIFF_TIME_COND="${CHUNKDIFF_TIME_COND:-1}"
export CHUNKDIFF_DENOISE_ADAPTER="${CHUNKDIFF_DENOISE_ADAPTER:-1}"

export COMPRESSOR="${COMPRESSOR:-brotli}"
export MATRIX_BITS="${MATRIX_BITS:-4}"
...
keys = [
    ...
    "MATRIX_LR", "MUON_WD", "EMA_DECAY", "WARMDOWN_FRAC", "GRAD_CLIP_NORM",
...
set +e
torchrun --standalone --nproc_per_node="${NPROC_PER_NODE:-8}" working/sp8192_chunkdiff_v3/train_gpt.py 2>&1 | tee "${LOG_PATH}"
