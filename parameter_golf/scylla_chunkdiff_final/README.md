...
Non-record 16MB lane experiment: Scylla tokenizer + full GPTQ/XSA/FA3 stack with a small training-only ChunkDiff denoising auxiliary loss.
...
The success metric is the final `final_int6_sliding_window_s64_exact` or `final_int6_roundtrip_exact` bpb, depending what the track expects.
