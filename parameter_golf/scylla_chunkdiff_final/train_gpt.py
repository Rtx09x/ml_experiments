...
class Hyperparameters:
    ...
    chunkdiff_enabled = bool(int(os.environ.get("CHUNKDIFF_ENABLED", "1")))
    chunkdiff_weight = float(os.environ.get("CHUNKDIFF_WEIGHT", "0.006"))
    chunkdiff_every = int(os.environ.get("CHUNKDIFF_EVERY", "4"))
    chunkdiff_seq_len = int(os.environ.get("CHUNKDIFF_SEQ_LEN", "256"))
...
class GPT(nn.Module):
    def __init__(
        ...
        self.chunkdiff_mask_embed = nn.Parameter(torch.zeros(1, 1, model_dim, dtype=torch.float32))
        self.chunkdiff_time_embed = nn.Embedding(8, model_dim)
    ...
    def _init_weights(self) -> None:
        ...
        nn.init.normal_(self.chunkdiff_time_embed.weight, mean=0.0, std=0.01)
    ...
    def chunkdiff_loss(
        ...
        """Training-only causal chunk denoising auxiliary objective."""
        bsz, full_len = input_ids.shape
        chunk_len = min(max(8, int(seq_len)), full_len)
        if chunk_len < full_len:
            start = int(torch.randint(0, full_len - chunk_len + 1, (1,), device=input_ids.device).item())
            ids = input_ids[:, start:start + chunk_len]
        else:
            ids = input_ids
        step_ids = torch.randint(0, max(int(steps), 1), (bsz,), device=input_ids.device)
        denom = max(int(steps) - 1, 1)
        mask_rate = mask_min + (mask_max - mask_min) * (step_ids.float() / denom)
        mask = torch.rand(ids.shape, device=ids.device) < mask_rate[:, None]
        mask[:, 0] = False
        ...
        if self.bigram is not None:
            x = x + self.bigram(ids)
        time_ids = torch.clamp(step_ids, max=self.chunkdiff_time_embed.num_embeddings - 1)
        x = torch.where(
            mask[..., None],
            self.chunkdiff_mask_embed.to(dtype=x.dtype) + self.chunkdiff_time_embed(time_ids).to(dtype=x.dtype)[:, None, :],
            x,
        )
        x = F.rms_norm(x, (x.size(-1),))
        x = self.smear(x)
        ...
        for i in range(self.num_decoder_layers):
            ...
            if skips:
                x = x + self.skip_weights[i].to(dtype=x.dtype)[None, None, :] * skips.pop()
            ve = self._get_ve(bi, ids, ve_cache)
            x = self.blocks[bi](x, x0,
                self.qo_bank[bi], self.kv_bank[bi], self.kv_bank[n + bi],
                self.qo_bank[n + bi], self.mlp_up_bank[bi], self.mlp_down_bank[bi],
                v_embed=ve)
        ...
        if self.tie_embeddings:
            ...
            logits_proj = self.lm_head(x)
        logits = self.logit_softcap * torch.tanh(logits_proj / self.logit_softcap)
        return F.cross_entropy(logits[mask].float(), ids[mask], reduction="mean")
...
def main() -> None:
    ...
    scalar_params.append(base_model.chunkdiff_mask_embed)
    scalar_params.append(base_model.chunkdiff_time_embed.weight)
    ...
    log0(
        ...
        f"every:{args.chunkdiff_every} seq_len:{args.chunkdiff_seq_len} "
        f"mask:[{args.chunkdiff_mask_min},{args.chunkdiff_mask_max}] steps:{args.chunkdiff_steps}"
    )
    log0(
    ...
    while True:
        ...
        for micro_step in range(grad_accum_steps):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                ar_loss = model(x, y)
                cd_loss = base_model.chunkdiff_loss(
                    x,
                    args.chunkdiff_seq_len,
                    args.chunkdiff_mask_min,
                    args.chunkdiff_mask_max,
                    args.chunkdiff_steps,
                ) if use_chunkdiff else ar_loss.new_zeros(())
                loss = ar_loss + args.chunkdiff_weight * cd_loss
            train_loss += loss.detach()
            train_chunkdiff_loss += cd_loss.detach()
