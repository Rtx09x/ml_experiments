...
class Hyperparameters:
    ...
    # Block diffusion auxiliary objective. This keeps normal AR BPB evaluation,
    # but trains selected contiguous token blocks with masked-denoising pressure.
    blockdiff_enabled = bool(int(os.environ.get("BLOCKDIFF_ENABLED", "1")))
    blockdiff_weight = float(os.environ.get("BLOCKDIFF_WEIGHT", 0.08))
    blockdiff_block_size = int(os.environ.get("BLOCKDIFF_BLOCK_SIZE", 16))
    blockdiff_mask_min = float(os.environ.get("BLOCKDIFF_MASK_MIN", 0.25))
    blockdiff_mask_max = float(os.environ.get("BLOCKDIFF_MASK_MAX", 0.75))
    blockdiff_every = int(os.environ.get("BLOCKDIFF_EVERY", 1))
    blockdiff_min_step = int(os.environ.get("BLOCKDIFF_MIN_STEP", 0))
...
class GPT(nn.Module):
    ...
    def blockdiff_loss(
        ...
        AR semantics are preserved across blocks, while the selected block uses
        bidirectional self-attention and masked denoising. Normal BPB evaluation
        remains purely causal and unchanged.
        """
        bsz, seqlen = token_ids.shape
        if block_size <= 1 or seqlen < block_size:
            return token_ids.new_zeros((), dtype=torch.float32)
        block_size = min(block_size, seqlen)
        num_blocks = seqlen // block_size
        usable = num_blocks * block_size
        if usable <= 0:
            return token_ids.new_zeros((), dtype=torch.float32)
        dev = token_ids.device
        block_ids = torch.randint(0, num_blocks, (bsz,), device=dev)
        pos = torch.arange(seqlen, device=dev)[None, :]
        block_start = (block_ids * block_size)[:, None]
        in_block = (pos >= block_start) & (pos < block_start + block_size)
        ...
        mask_positions = in_block & random_mask
        if not bool(mask_positions.any()):
            mask_positions = in_block & (pos == block_start)
        qpos = torch.arange(seqlen, device=dev)[None, :, None]
        kpos = torch.arange(seqlen, device=dev)[None, None, :]
        block_start3 = block_start[:, :, None]
        block_end3 = block_start3 + block_size
        q_in_block = (qpos >= block_start3) & (qpos < block_end3)
        k_in_block = (kpos >= block_start3) & (kpos < block_end3)
        causal = kpos <= qpos
        blockdiff_attn_mask = (causal | (q_in_block & k_in_block))[:, None, :, :]
        x = self._run_layers(
            token_ids,
            mask_positions=mask_positions,
            attn_mask=blockdiff_attn_mask,
        )
        if self.tie_embeddings:
            logits_proj = F.linear(x, self.tok_emb.weight)
        else:
...
def main() -> None:
    ...
    log0(
        f"blockdiff:enabled:{int(args.blockdiff_enabled)} weight:{args.blockdiff_weight} "
        f"block_size:{args.blockdiff_block_size} mask:[{args.blockdiff_mask_min},{args.blockdiff_mask_max}] "
        f"every:{args.blockdiff_every} min_step:{args.blockdiff_min_step}"
    )
    log0(
    ...
    while True:
        ...
        train_blockdiff_loss = torch.zeros((), device=device)
        use_blockdiff = (
            args.blockdiff_enabled
            and args.blockdiff_weight > 0.0
            and step >= args.blockdiff_min_step
            and args.blockdiff_every > 0
            and step % args.blockdiff_every == 0
        )
        for micro_step in range(grad_accum_steps):
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                ar_loss = model(x, y)
                if use_blockdiff:
                    blockdiff_loss = base_model.blockdiff_loss(
                        x,
                        block_size=args.blockdiff_block_size,
                        mask_min=args.blockdiff_mask_min,
                        mask_max=args.blockdiff_mask_max,
                    )
                    loss = ar_loss + args.blockdiff_weight * blockdiff_loss
                else:
                    blockdiff_loss = torch.zeros((), device=device)
                    loss = ar_loss
            train_loss += loss.detach()
            train_blockdiff_loss += blockdiff_loss.detach()
            (loss * grad_scale).backward()
        train_loss /= grad_accum_steps
        train_blockdiff_loss /= grad_accum_steps
        ...
        if should_log_train:
            ...
            log0(
                ...
                f"blockdiff_loss:{train_blockdiff_loss.item():.4f} "
