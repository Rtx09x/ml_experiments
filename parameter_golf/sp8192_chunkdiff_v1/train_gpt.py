...
class GPT(nn.Module):
	def __init__(self,h):
		...
		self.tie_embeddings=h.tie_embeddings;self.tied_embed_init_std=h.tied_embed_init_std;self.logit_softcap=h.logit_softcap;self.tok_emb=nn.Embedding(h.vocab_size,h.embedding_dim);self.blockdiff_mask_embed=nn.Parameter(torch.zeros(h.embedding_dim,dtype=torch.float32))
	...
	def chunkdiff_loss(self,input_ids,chunk_size,mask_min,mask_max):
		...
		mask_min=max(0.,min(float(mask_min),1.));mask_max=max(mask_min,min(float(mask_max),1.));ratios=torch.empty((bsz,1),device=dev).uniform_(mask_min,mask_max);mask_positions=in_chunk&(torch.rand((bsz,seqlen),device=dev)<ratios)
		...
		x=self.tok_emb(input_ids);x=torch.where(mask_positions[...,None],self.blockdiff_mask_embed.to(dtype=x.dtype,device=dev)[None,None,:],x);x=F.rms_norm(x,(x.size(-1),))
		if self.embed_proj is not None:x=self.embed_proj(x)
		x0=x;skips=[];enc_iter=self.encoder_indices if self.looping_active else range(self.num_encoder_layers);dec_iter=self.decoder_indices if self.looping_active else range(self.num_encoder_layers,self.num_encoder_layers+self.num_decoder_layers)
