...
EXPERIMENT_DIR="${EXPERIMENT_REPO_DIR}/parameter_golf/sp8192_chunkdiff_v1"
RUN_ID="${RUN_ID:-sp8192_chunkdiff_v1_8xh100}"
RUN_ROOT="${RUN_ROOT:-${WORKSPACE_ROOT}/sp8192_chunkdiff_v1_runs}"
...
mkdir -p "${PARAMETER_GOLF_DIR}/working/sp8192_chunkdiff_v1"
cp "${EXPERIMENT_DIR}/train_gpt.py" "${PARAMETER_GOLF_DIR}/working/sp8192_chunkdiff_v1/train_gpt.py"
cp "${EXPERIMENT_DIR}/README.md" "${PARAMETER_GOLF_DIR}/working/sp8192_chunkdiff_v1/README.md"
cp "${EXPERIMENT_DIR}/runpod_8xh100.sh" "${PARAMETER_GOLF_DIR}/working/sp8192_chunkdiff_v1/runpod_8xh100.sh"
...
export SPARSE_TARGET="${SPARSE_TARGET:-0.40}"
...
export CHUNKDIFF_MASK_MIN="${CHUNKDIFF_MASK_MIN:-0.35}"
export CHUNKDIFF_MASK_MAX="${CHUNKDIFF_MASK_MAX:-0.80}"
export CHUNKDIFF_EVERY="${CHUNKDIFF_EVERY:-2}"
export CHUNKDIFF_MIN_STEP="${CHUNKDIFF_MIN_STEP:-0}"
export CHUNKDIFF_SEQ_LEN="${CHUNKDIFF_SEQ_LEN:-512}"
...
keys = [
    ...
    "SPARSE_END_STEP", "SPARSE_EVERY", "SPARSE_INCLUDE", "CHUNKDIFF_ENABLED", "CHUNKDIFF_WEIGHT", "CHUNKDIFF_CHUNK_SIZE", "CHUNKDIFF_MASK_MIN", "CHUNKDIFF_MASK_MAX", "CHUNKDIFF_EVERY", "CHUNKDIFF_MIN_STEP", "CHUNKDIFF_SEQ_LEN", "COMPRESSOR",
...
set +e
torchrun --standalone --nproc_per_node="${NPROC_PER_NODE:-8}" working/sp8192_chunkdiff_v1/train_gpt.py 2>&1 | tee "${LOG_PATH}"
