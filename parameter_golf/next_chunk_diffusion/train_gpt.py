...
class Hyperparameters:
    ...
    # GPTQ/int6 export. Keep export configurable because research runs should
    # not burn GPU money in packaging code when the BPB is already bad.
    gptq_export = bool(int(os.environ.get("GPTQ_EXPORT", "1")))
    fast_export = bool(int(os.environ.get("FAST_EXPORT", "1")))
    quant_roundtrip_eval = bool(int(os.environ.get("QUANT_ROUNDTRIP_EVAL", "1")))
    selective_prune_mode = os.environ.get("SELECTIVE_PRUNE_MODE", "full").strip().lower()
    lzma_preset = int(os.environ.get("LZMA_PRESET", "6" if fast_export else "9"))
    gptq_calib_batches = int(os.environ.get("GPTQ_CALIB_BATCHES", 64))
    gptq_block_size = int(os.environ.get("GPTQ_BLOCK_SIZE", 128))
    # Next-chunk diffusion auxiliary objective. This keeps normal AR BPB
    # evaluation, but trains a future chunk with masked-denoising pressure while
    # preventing visibility into later chunks.
    chunkdiff_enabled = bool(int(os.environ.get("CHUNKDIFF_ENABLED", os.environ.get("BLOCKDIFF_ENABLED", "1"))))
    chunkdiff_weight = float(os.environ.get("CHUNKDIFF_WEIGHT", os.environ.get("BLOCKDIFF_WEIGHT", 0.05)))
    chunkdiff_chunk_size = int(os.environ.get("CHUNKDIFF_CHUNK_SIZE", os.environ.get("BLOCKDIFF_BLOCK_SIZE", 64)))
    chunkdiff_mask_min = float(os.environ.get("CHUNKDIFF_MASK_MIN", os.environ.get("BLOCKDIFF_MASK_MIN", 0.50)))
    chunkdiff_mask_max = float(os.environ.get("CHUNKDIFF_MASK_MAX", os.environ.get("BLOCKDIFF_MASK_MAX", 0.95)))
    chunkdiff_every = int(os.environ.get("CHUNKDIFF_EVERY", os.environ.get("BLOCKDIFF_EVERY", 1)))
    chunkdiff_min_step = int(os.environ.get("CHUNKDIFF_MIN_STEP", os.environ.get("BLOCKDIFF_MIN_STEP", 0)))
    # Magnitude sparsity applied during training and again before export.
    sparse_enabled = bool(int(os.environ.get("SPARSE_ENABLED", "1")))
    sparse_target = float(os.environ.get("SPARSE_TARGET", 0.50))
    sparse_start_step = int(os.environ.get("SPARSE_START_STEP", 1000))
...
class Muon(torch.optim.Optimizer):
    ...
    def __init__(self, params, lr: float, momentum: float, backend_steps: int,
                 nesterov: bool = True, weight_decay: float = 0.0, eq_r: bool = False):
        super().__init__(
            params,
            dict(lr=lr, momentum=momentum, backend_steps=backend_steps,
                 nesterov=nesterov, weight_decay=weight_decay, eq_r=eq_r),
        )
        self._built = False
...
class Block(nn.Module):
    ...
    def forward(
        ...
        if not self.parallel:
            ...
            x_in = mix[0][None, None, :] * x_attn + mix[1][None, None, :] * x0
        ...
        attn_out, raw_v = self.attn(
            self.attn_norm(x_in_attn) * self.ln_scale_factor,
            q_w,
            k_w,
            v_w,
            out_w,
...
class GPT(nn.Module):
    def __init__(
        ...
        self.blockdiff_mask_embed = nn.Parameter(torch.zeros(model_dim, dtype=torch.float32))
    ...
    def _run_block(
        ...
        return block(
            ...
            v_embed=ve, v0=v0, attn_mask=attn_mask,
        )
    def _run_layers(
        self,
        input_ids: Tensor,
        ...
        if mask_positions is not None:
            mask_embed = self.blockdiff_mask_embed.to(dtype=x.dtype, device=x.device)
            x = torch.where(mask_positions[..., None], mask_embed[None, None, :], x)
        if self.bigram is not None:
            bigram_x = self.bigram(input_ids)
            if mask_positions is not None:
                bigram_x = torch.where(mask_positions[..., None], torch.zeros_like(bigram_x), bigram_x)
            x = x + bigram_x
        ...
        for i in range(self.num_encoder_layers):
            x_attn, x_mlp, raw_v = self._run_block(
                self.blocks[i], i, x_attn, x_mlp, x0, input_ids, ve_cache, v0, attn_mask=attn_mask
            )
            if v0 is None and raw_v is not None:
                v0 = raw_v
            if i in self.recur_layer_ids:
                up_w = self.recur_mlp_up[str(i)] if self.recur_mlp_up is not None else self.mlp_up_bank[i]
                down_w = self.recur_mlp_down[str(i)] if self.recur_mlp_down is not None else self.mlp_down_bank[i]
                x_attn, x_mlp, _ = self._run_block(
                    self.blocks[i], i, x_attn, x_mlp, x0, input_ids, ve_cache, v0,
                    up_w=up_w, down_w=down_w, attn_mask=attn_mask
                )
            skips.append(x_attn)
        for i in range(self.num_decoder_layers):
            ...
            x_attn, x_mlp, _ = self._run_block(
                self.blocks[bi], bi, x_attn, x_mlp, x0, input_ids, ve_cache, v0, attn_mask=attn_mask
            )
            if bi in self.recur_layer_ids:
                up_w = self.recur_mlp_up[str(bi)] if self.recur_mlp_up is not None else self.mlp_up_bank[bi]
                down_w = self.recur_mlp_down[str(bi)] if self.recur_mlp_down is not None else self.mlp_down_bank[bi]
                x_attn, x_mlp, _ = self._run_block(
                    self.blocks[bi], bi, x_attn, x_mlp, x0, input_ids, ve_cache, v0,
                    up_w=up_w, down_w=down_w, attn_mask=attn_mask
                )
        if self.lane_merge is not None:
            merge = self.lane_merge.to(dtype=x_attn.dtype)
            x = merge * x_attn + (1.0 - merge) * x_mlp
        else:
            x = x_attn
    ...
    def chunkdiff_loss(
        ...
        """Next-chunk diffusion training loss.

        A future chunk is selected per sequence. Positions inside the selected
        chunk can use bidirectional self-attention during denoising, while all
        later positions remain invisible to that chunk. Normal BPB evaluation
        remains purely causal and unchanged.
        """
        bsz, seqlen = token_ids.shape
        if chunk_size <= 1 or seqlen < chunk_size:
            return token_ids.new_zeros((), dtype=torch.float32)
        chunk_size = min(chunk_size, seqlen)
        num_chunks = seqlen // chunk_size
        usable = num_chunks * chunk_size
        if usable <= 0:
            return token_ids.new_zeros((), dtype=torch.float32)
        dev = token_ids.device
        # Prefer a true "next chunk" task with at least one previous chunk as
        # context. Fall back to chunk 0 only for tiny smoke configurations.
        first_chunk = 1 if num_chunks > 1 else 0
        chunk_ids = torch.randint(first_chunk, num_chunks, (bsz,), device=dev)
        pos = torch.arange(seqlen, device=dev)[None, :]
        chunk_start = (chunk_ids * chunk_size)[:, None]
        in_chunk = (pos >= chunk_start) & (pos < chunk_start + chunk_size)
        mask_min = max(0.0, min(mask_min, 1.0))
        mask_max = max(mask_min, min(mask_max, 1.0))
        ratios = torch.empty((bsz, 1), device=dev).uniform_(mask_min, mask_max)
        random_mask = torch.rand((bsz, seqlen), device=dev) < ratios
        mask_positions = in_chunk & random_mask
        if not bool(mask_positions.any()):
            mask_positions = in_chunk & (pos == chunk_start)
        qpos = torch.arange(seqlen, device=dev)[None, :, None]
        kpos = torch.arange(seqlen, device=dev)[None, None, :]
        chunk_start3 = chunk_start[:, :, None]
        chunk_end3 = chunk_start3 + chunk_size
        q_in_chunk = (qpos >= chunk_start3) & (qpos < chunk_end3)
        k_in_chunk = (kpos >= chunk_start3) & (kpos < chunk_end3)
        causal = kpos <= qpos
        chunkdiff_attn_mask = (causal | (q_in_chunk & k_in_chunk))[:, None, :, :]
        x = self._run_layers(
            token_ids,
            mask_positions=mask_positions,
            attn_mask=chunkdiff_attn_mask,
        )
        if self.tie_embeddings:
            logits_proj = F.linear(x, self.tok_emb.weight)
        else:
            ...
            logits_proj = self.lm_head(x)
        logits = self.logit_softcap * torch.tanh(logits_proj / self.logit_softcap)
        return F.cross_entropy(logits[mask_positions].float(), token_ids[mask_positions], reduction="mean")
...
def _sparse_target_for_step(args: Hyperparameters, step: int) -> float:
    if not args.sparse_enabled or args.sparse_target <= 0.0:
        return 0.0
    if step < args.sparse_start_step:
        return 0.0
    if args.sparse_end_step <= args.sparse_start_step:
        return max(0.0, min(args.sparse_target, 0.99))
    frac = (step - args.sparse_start_step) / max(args.sparse_end_step - args.sparse_start_step, 1)
    frac = max(0.0, min(frac, 1.0))
    return max(0.0, min(args.sparse_target * frac, 0.99))

@torch.no_grad()
def apply_magnitude_sparsity_(model: nn.Module, args: Hyperparameters, target: float) -> tuple[int, int]:
    """Zero smallest-magnitude bank weights for compression-oriented sparse tickets."""
    if target <= 0.0:
        return 0, 0
    include = {x.strip().lower() for x in args.sparse_include.split(",") if x.strip()}
    tensors: list[Tensor] = []
    if "attn" in include:
        tensors.extend([model.qo_bank, model.kv_bank])
    if "mlp" in include:
        tensors.extend([model.mlp_up_bank, model.mlp_down_bank])
        if getattr(model, "recur_mlp_up", None) is not None:
            for lid in model.recur_layer_ids:
                tensors.append(model.recur_mlp_up[str(lid)])
                tensors.append(model.recur_mlp_down[str(lid)])
    total = sum(t.numel() for t in tensors)
    if total <= 0:
        return 0, 0
    zeros = 0
    for t in tensors:
        flat = t.data.float().abs().flatten()
        k = int(target * flat.numel())
        if k <= 0:
            zeros += int((t.data == 0).sum().item())
            continue
        if k >= flat.numel():
            t.data.zero_()
            zeros += t.numel()
            continue
        threshold = torch.kthvalue(flat, k).values.to(device=t.device, dtype=t.dtype)
        t.data.masked_fill_(t.data.abs() <= threshold, 0)
        zeros += int((t.data == 0).sum().item())
    return zeros, total
...
class _HessianBlock(nn.Module):
    def __init__(self, dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init, layer_idx=0, ln_scale=False, parallel: bool = False):
        super().__init__()
        self.attn_norm = RMSNorm()
        self.mlp_norm = RMSNorm()
        ...
        if parallel:
            self.resid_mix_mlp = nn.Parameter(torch.stack((torch.ones(dim), torch.zeros(dim))).float())
            self.route = nn.Parameter(torch.tensor([1.0, 1.0, 1.0, 1.0]))
    def forward(self, x_attn, x_mlp, x0, v_embed=None):
        if not self.parallel:
            mix = self.resid_mix.to(dtype=x_attn.dtype)
            x_in = mix[0][None, None, :] * x_attn + mix[1][None, None, :] * x0
            ...
            return x_out, x_out
        route = self.route.to(dtype=x_attn.dtype)
        mix_attn = self.resid_mix.to(dtype=x_attn.dtype)
        x_in_attn = mix_attn[0][None, None, :] * x_attn + mix_attn[1][None, None, :] * x0
        attn_out = self.attn(self.attn_norm(x_in_attn) * self.ln_scale_factor, v_embed=v_embed)
        attn_delta = self.attn_scale.to(dtype=x_attn.dtype)[None, None, :] * attn_out
        mix_mlp = self.resid_mix_mlp.to(dtype=x_mlp.dtype)
        x_in_mlp = mix_mlp[0][None, None, :] * x_mlp + mix_mlp[1][None, None, :] * x0
        mlp_delta = self.mlp_scale.to(dtype=x_mlp.dtype)[None, None, :] * self.mlp(self.mlp_norm(x_in_mlp) * self.ln_scale_factor)
        x_attn_out = x_attn + route[0] * attn_delta + route[2] * mlp_delta
        x_mlp_out = x_mlp + route[1] * attn_delta + route[3] * mlp_delta
        return x_attn_out, x_mlp_out
...
class _HessianGPT(nn.Module):
    ...
    def __init__(self, vocab_size, num_layers, model_dim, num_heads, num_kv_heads,
                 ...
                 smear_enabled=False, parallel_start_layer: int = 0,
                 recur_layers: str = "", recur_untie_mlp: bool = True):
        super().__init__()
        self.tie_embeddings = tie_embeddings
        self.logit_softcap = logit_softcap
        self.num_layers = num_layers
        ...
        self.blocks = nn.ModuleList([
            _HessianBlock(model_dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init,
                          layer_idx=i, ln_scale=ln_scale, parallel=(parallel_start_layer > 0 and i >= parallel_start_layer))
            for i in range(num_layers)
        ])
        if rope_dims > 0:
            head_dim = model_dim // num_heads
        ...
        if self.recur_layer_ids and recur_untie_mlp:
            ...
            self.recur_mlp_up = None
            self.recur_mlp_down = None
        self.lane_merge = nn.Parameter(torch.tensor(0.5)) if parallel_start_layer > 0 else None
    ...
    def _run_layers(self, input_ids):
        x = self.tok_emb(input_ids)
        if self.bigram is not None:
        ...
        for i in range(self.num_encoder_layers):
            ve = self._get_ve(i, input_ids, ve_cache)
            x_attn, x_mlp = self.blocks[i](x_attn, x_mlp, x0, v_embed=ve)
            if i in self.recur_layer_ids:
                x_attn, x_mlp = self.blocks[i](x_attn, x_mlp, x0, v_embed=ve)
            skips.append(x_attn)
        for i in range(self.num_decoder_layers):
            ...
            x_attn, x_mlp = self.blocks[bi](x_attn, x_mlp, x0, v_embed=ve)
            if bi in self.recur_layer_ids:
                x_attn, x_mlp = self.blocks[bi](x_attn, x_mlp, x0, v_embed=ve)
        if self.lane_merge is not None:
            merge = self.lane_merge.to(dtype=x_attn.dtype)
...
def main() -> None:
    ...
    if base_model.recur_mlp_up is not None:
        for lid in base_model.recur_layer_ids:
            base_model.recur_mlp_up[str(lid)].data = base_model.recur_mlp_up[str(lid)].data.float()
            base_model.recur_mlp_down[str(lid)].data = base_model.recur_mlp_down[str(lid)].data.float()
    ...
    scalar_params.append(base_model.blockdiff_mask_embed)
    ...
    log0(
        f"parallel_start_layer:{args.parallel_start_layer} recur_layers:{args.recur_layers or 'none'} "
        f"recur_untie_mlp:{int(args.recur_untie_mlp)} muon_eq_r:{int(args.muon_eq_r)}"
    )
    log0(
        f"chunkdiff:enabled:{int(args.chunkdiff_enabled)} weight:{args.chunkdiff_weight} "
        f"chunk_size:{args.chunkdiff_chunk_size} mask:[{args.chunkdiff_mask_min},{args.chunkdiff_mask_max}] "
        f"every:{args.chunkdiff_every} min_step:{args.chunkdiff_min_step}"
    )
    log0(
        f"sparsity:enabled:{int(args.sparse_enabled)} target:{args.sparse_target} "
        f"schedule:{args.sparse_start_step}->{args.sparse_end_step} every:{args.sparse_every} "
        f"include:{args.sparse_include}"
    )
    log0(
        f"export:gptq:{int(args.gptq_export)} fast:{int(args.fast_export)} "
        f"selective_prune_mode:{args.selective_prune_mode} lzma_preset:{args.lzma_preset} "
        f"roundtrip_eval:{int(args.quant_roundtrip_eval)}"
    )
    ...
    while True:
        ...
        train_chunkdiff_loss = torch.zeros((), device=device)
        use_chunkdiff = (
            args.chunkdiff_enabled
            and args.chunkdiff_weight > 0.0
            and step >= args.chunkdiff_min_step
            and args.chunkdiff_every > 0
            and step % args.chunkdiff_every == 0
        )
        for micro_step in range(grad_accum_steps):
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                ar_loss = model(x, y)
                if use_chunkdiff:
                    chunkdiff_loss = base_model.chunkdiff_loss(
                        x,
                        chunk_size=args.chunkdiff_chunk_size,
                        mask_min=args.chunkdiff_mask_min,
                        mask_max=args.chunkdiff_mask_max,
                    )
                    loss = ar_loss + args.chunkdiff_weight * chunkdiff_loss
                else:
                    chunkdiff_loss = torch.zeros((), device=device)
                    loss = ar_loss
            train_loss += loss.detach()
            train_chunkdiff_loss += chunkdiff_loss.detach()
            (loss * grad_scale).backward()
        train_loss /= grad_accum_steps
        train_chunkdiff_loss /= grad_accum_steps
        ...
        optimizer_muon.step()
        zero_grad_all()
        if args.sparse_enabled and args.sparse_every > 0 and step % args.sparse_every == 0:
            sparse_target = _sparse_target_for_step(args, step)
            sparse_zeros, sparse_total = apply_magnitude_sparsity_(base_model, args, sparse_target)
            if sparse_total > 0 and (step <= args.sparse_start_step or step % max(args.sparse_every * 10, 1) == 0):
                log0(
                    f"sparsity:step:{step} target:{sparse_target:.3f} "
                    f"actual:{sparse_zeros / max(sparse_total, 1):.3f} include:{args.sparse_include}"
                )
        # EMA update
        with torch.no_grad():
        ...
        if should_log_train:
            ...
            tok_per_sec = tokens_seen / max(elapsed_s, 1e-9)
            pct = 100.0 * min(approx_training_time_ms / max(max_wallclock_ms or approx_training_time_ms, 1.0), 1.0)
            eta_s = 0.0 if max_wallclock_ms is None else max(max_wallclock_ms - approx_training_time_ms, 0.0) / 1000.0
            log0(
                f"step:{step}/{args.iterations} train_loss:{train_loss.item():.4f} "
                f"chunkdiff_loss:{train_chunkdiff_loss.item():.4f} "
                f"train_time:{approx_training_time_ms:.0f}ms step_avg:{approx_training_time_ms / step:.2f}ms "
                f"tok/s:{tok_per_sec:.0f} progress:{pct:.1f}% eta:{eta_s/60.0:.1f}m"
            )
    ...
    if args.lawa_enabled and len(lawa_queue) > 1:
        ...
        base_model.load_state_dict(avg_state, strict=True)
    if args.sparse_enabled and args.sparse_target > 0.0:
        sparse_zeros, sparse_total = apply_magnitude_sparsity_(base_model, args, args.sparse_target)
        log0(
            f"sparsity:final target:{args.sparse_target:.3f} "
            f"actual:{sparse_zeros / max(sparse_total, 1):.3f} include:{args.sparse_include}"
        )
    torch.cuda.synchronize()
    ...
    if master_process:
        ...
        torch.save(export_sd, "final_model_pre_quant.pt")
        model_bytes = os.path.getsize("final_model.pt")
        code_bytes = len(code.encode("utf-8"))
        log0(f"Serialized model: {model_bytes} bytes")
        log0(f"Saved pre-quant model checkpoint: final_model_pre_quant.pt")
        log0(f"Code size: {code_bytes} bytes")
    if not args.gptq_export:
        log0("gptq:disabled; skipping int6 export and quantized roundtrip eval")
        if distributed:
    ...
    legacy_selective_prune = args.selective_prune_mode in {"binary", "search"}
    if args.selective_prune_mode in {"none", "off", "skip", "0", "false"}:
        log0("selective_prune:skipped mode:none")
    elif args.selective_prune_mode in {"full", "all", "fast"}:
        ...
        for name, info in quant_meta.items():
            ...
            q = tmp[qk]
            ones_mask = q.abs() == 1
            count = int(ones_mask.sum().item())
            if count:
                q[ones_mask] = 0
                pruned += count
        buf = io.BytesIO()
        torch.save({"w": tmp, "m": quant_meta}, buf)
        full_sz = len(lzma.compress(buf.getvalue(), preset=args.lzma_preset)) + code_bytes_est
        log0(
            f"selective_prune:mode:{args.selective_prune_mode} pruned_all_pm1:{pruned} "
            f"size:{full_sz/(1024*1024):.2f}MB target:{target_mb}MB"
        )
        quant_result = tmp
    elif not legacy_selective_prune:
        raise ValueError(f"Unknown SELECTIVE_PRUNE_MODE={args.selective_prune_mode!r}")
    ones_info = []  # (tensor_key, flat_idx, error)
    for name, info in (() if not legacy_selective_prune else quant_meta.items()):
    ...
    if not args.quant_roundtrip_eval:
        log0("final_int6_roundtrip:skipped")
        if distributed:
            dist.destroy_process_group()
        return
    if distributed:
    ...
    if eval_model.recur_mlp_up is not None:
        for lid in eval_model.recur_layer_ids:
            eval_model.recur_mlp_up[str(lid)].data = eval_model.recur_mlp_up[str(lid)].data.float()
            eval_model.recur_mlp_down[str(lid)].data = eval_model.recur_mlp_down[str(lid)].data.float()
